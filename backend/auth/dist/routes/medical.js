...
async function authMiddleware(req, res, next) {
    ...
    try {
        if (role === 'doctor') {
            const doc = await Doctor_1.default.findById(id);
            if (!doc)
            ...
            const pat = await Patient_1.default.findById(id);
            if (!pat)
...
function sanitizeDoctor(doc) {
    ...
    const slotsIso = Array.isArray(doc.availabilitySlots)
        ? doc.availabilitySlots.map((s) => s.datetime instanceof Date
...
router.get('/doctors', async (req, res) => {
    try {
        ...
        const docs = await Doctor_1.default.find(filter)
...
router.get('/doctors/:id', async (req, res) => {
    try {
        const doc = await Doctor_1.default.findById(req.params.id);
        if (!doc)
...
router.get('/doctor/me', authMiddleware, async (req, res) => {
    ...
    try {
        const doc = await Doctor_1.default.findById(req.user.id);
        if (!doc)
            return res.status(404).json({ message: 'Doctor not found' });
        const now = new Date();
        const slotsIso = Array.isArray(doc.availabilitySlots)
        ...
        const nextSlots = future.slice(0, 5).map((d) => d.toISOString());
        // total earnings
        const earningsAgg = await Appointment_1.default.aggregate([
            {
                $match: {
                    doctor: new mongoose_1.default.Types.ObjectId(doc._id),
                    status: 'completed',
                    paymentStatus: 'paid',
                },
            },
...
router.get('/doctor/me/profile-image', authMiddleware, async (req, res) => {
    if (!req.user || req.user.role !== 'doctor') {
        return res.status(403).json({ message: 'Access denied' });
    }
    try {
        const doc = await Doctor_1.default.findById(req.user.id).select('profileImage');
...
router.put('/doctor/me', authMiddleware, upload.single('profileImage'), async (req, res) => {
    if (!req.user || req.user.role !== 'doctor') {
        return res.status(403).json({ message: 'Access denied' });
    }
    try {
        ...
        if (!doc)
            return res.status(404).json({ message: 'Doctor not found' });
        const { name, email, specialty, phone, dob, locationObj, availabilitySlots, maxPatients, experience, hospitalAffiliation, bio, qualifications, languages, consultationFee, } = req.body;
        // Simple fields
        if (name)
            doc.name = name;
        if (email)
        ...
        if (availabilitySlots) {
            ...
            if (Array.isArray(arr)) {
                doc.availabilitySlots = arr
                    .map((s) => {
                    const d = new Date(typeof s === 'string' ? s : s?.datetime);
                    if (isNaN(d.getTime()))
                        return null;
                    return {
                        datetime: d,
                        quantity: typeof s === 'object' && 'quantity' in s
                            ? s.quantity
                            : undefined,
                    };
                })
        ...
        if (qualifications) {
            try {
                const q = typeof qualifications === 'string'
        ...
        if (languages) {
            try {
                const l = typeof languages === 'string'
        ...
        if (hospitalAffiliation)
            doc.hospitalAffiliation = hospitalAffiliation;
        if (bio)
            doc.bio = bio;
        ...
        return res.json({
            ...
            locationObj: doc.locationObj || null,
            availabilitySlots: doc.availabilitySlots.map((s) => s.datetime.toISOString()),
            maxPatients: doc.maxPatients,
            experience: doc.experience,
            hospitalAffiliation: doc.hospitalAffiliation,
            bio: doc.bio,
...
router.delete('/doctor/me', authMiddleware, async (req, res) => {
    ...
    try {
        await Doctor_1.default.findByIdAndDelete(req.user.id);
...
router.get('/doctor/:id/appointments', authMiddleware, async (req, res) => {
    if (!req.user ||
